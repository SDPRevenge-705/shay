...
public class Test11 {
    public static void main(String[] args) {
        ...
        //判断蓝球
        int blueumber = userInputArr[userInputArr.length -1];
        if (blueumber == arr[arr.length -1]){
            blueCount++;
        }
    ...
    public static int[] userInputNumber(){
        ...
        for (int i = 0; i < 6; ) {
            ...
            //redNumber 在 1～33 唯一不重复
            if (redNumber >= 1 && redNumber <= 33) {
                boolean flag = contians(arr,redNumber);
                if (!flag) {
                    //不存在
                    //有效的，可以添加到数组当中
                    arr[i] = redNumber;
                    ...
                    //存在
                    System.out.println("当前红球号码重复错误");
                }
            }else{
                System.out.println("当前红球号码超出错误");
            }
        }
    ...
    //用于判断数组在数组中是否存在
    public static boolean contians(int[] arr,int number){
        for (int i = 0; i < arr.length; i++) {
            if (arr[i]==number) {
                return true;
            }
